...
* Modules: `app` (startup, app\_id), `ui` (widgets/layout), `text` (Arabic rendering helpers), `data` (Qur'an/translation models, index, search), `state` (navigation, settings), `storage` (bookmarks, progress), `config` (paths, preferences), `logging` (tracing)
