use crate::state::Surah;

pub fn default_surahs() -> Vec<Surah> {
    let names_en = [
    ...
    (1..=114)
        .map(|i| Surah {
            id: i,
            name_ar: names_ar[(i - 1) as usize].to_string(),
            name_en: names_en[(i - 1) as usize].to_string(),
            ayah_count: counts[(i - 1) as usize] as u16,
        })
        .collect()
}
